...
def parse_video_filename(filename: str) -> Tuple[Optional[str], Optional[datetime], Optional[str]]:
    ...
    if match:
        job_id = match.group(1)
        # Check if job_id looks like a timestamp
        if job_id.isdigit() and len(job_id) >= 10:
            try:
                # Time in seconds
                timestamp_val = int(job_id)
                timestamp = datetime.fromtimestamp(timestamp_val, tz=timezone.utc)
                return job_id, timestamp, "chad_goldstein"
            except (ValueError, OSError):
                pass
        return job_id, None, "chad_goldstein"
...
def create_job_from_s3_video(job_storage, s3_key: str, file_info: Dict, 
    ...
    try:
        ...
        # Create the job with the correct ID
        job_data["id"] = job_id
        
        # Use the storage's create_job method for consistency
        # For Redis, we need to handle the active jobs set manually
        if hasattr(job_storage, 'redis') and job_storage.redis:
            # Redis-specific: store directly and add to active set
            job_storage.redis.set(f"job:{job_id}", json.dumps(job_data))
            job_storage.redis.sadd("jobs:active", job_id)
        else:
            # Firestore: store directly with the specific job_id
...
def main():
    parser = argparse.ArgumentParser(description='Sync S3 videos to production job storage')
    parser.add_argument('--dry-run', action='store_true', 
                       help='List videos and jobs without creating new ones')
    parser.add_argument('--storage-type', default='redis', choices=['redis', 'firestore'],
                       help='Job storage type (redis or firestore)')
    parser.add_argument('--redis-url', default='redis://redis:6379',
                       help='Redis URL for production Redis')
    parser.add_argument('--firestore-project-id', 
    ...
    try:
        ...
        logger.info(f"Initializing {args.storage_type} job storage...")
        ...
        for s3_key in video_keys:
            try:
                ...
                # Get all jobs from storage (implementation depends on storage type)
                jobs_to_check = []
                if hasattr(job_storage, 'redis') and job_storage.redis:
                    # Redis-specific: get active job IDs
                    ...
                    for existing_job_id in job_ids:
                        ...
                        if existing_job_data:
                            jobs_to_check.append(existing_job_data)
                elif hasattr(job_storage, 'list_jobs'):
                    # Firestore: use list_jobs method
                    jobs_to_check = job_storage.list_jobs(limit=1000)  # Get all jobs
                
                # Check each job for matching video paths
                for existing_job_data in jobs_to_check:
                ...
                if existing_job:
                    logger.info(f"Job already exists in {args.storage_type} for video {s3_key} (job_id: {existing_job.get('id', 'unknown')})")
                    existing_count += 1
                    continue
        ...
        logger.info(f"Jobs already exist in {args.storage_type}: {existing_count}")
